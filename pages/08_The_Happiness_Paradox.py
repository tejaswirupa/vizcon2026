...
<style>
.hero-insight { background:linear-gradient(135deg,#2a1a3a,#3d2d5e); border:2px solid #f39c12;
    border-radius:12px; padding:2rem; margin:1.5rem 0; text-align:center; }
.hero-text { font-size:1.3rem; color:#ffffff; font-weight:600; line-height:1.7; }
.conclusion-box { background:linear-gradient(135deg,#1a2a1a,#2d3d2d); border:1px solid #2ecc71;
    border-radius:12px; padding:2rem; margin:1.5rem 0; }
.cta-box { background:linear-gradient(135deg,#3a1a1a,#5e2d2d); border:2px solid #e74c3c;
    border-radius:12px; padding:1.5rem; margin:1rem 0; text-align:center; }
.about-card { background:linear-gradient(135deg,#1a1a3e,#2d2d5e); border:1px solid #4a4a8a;
...
st.markdown('''<div class="page-context">
    ...
    If sustainability and happiness were compatible, we would expect to see a cluster of countries
    in the top-left quadrant — high happiness, low footprint.
    What we actually find is almost the opposite:
    <strong style="color:#e74c3c">the happiest countries are clustered in the top-right</strong>
    — high happiness, high ecological cost.<br><br>
    The red dashed line at x=1 marks the planetary boundary.
    <strong style="color:#ffffff">Every country to the right of that line is borrowing against the future.</strong>
    India, the only country to the left, sits alone at the bottom — sustainable but struggling.<br><br>
    Brazil offers the closest thing to hope in the data: moderate happiness (6.55)
    with a relatively low footprint (1.69 Earths). It is imperfect, but it points
    toward a <strong style="color:#2ecc71">third path</strong>.
</div>''', unsafe_allow_html=True)
...
st.markdown('''<div class="hero-insight">
    <div class="hero-text">
        ...
        <span style="color:#f39c12; font-size:1.5rem; font-weight:900">
...
facts = [
    ("🇦🇺 7.0", "#f39c12", "Happiness — needs 3.83 Earths"),
    ("🇧🇷 6.6", "#2ecc71", "Happiness — 1.69 Earths (Sweet Spot)"),
    ("🇨🇦 6.8", "#e74c3c", "Happiness — needs 4.91 Earths"),
    ("🇮🇳 4.7", "#3498db", "Happiness — 0.69 Earths (Sustainable)"),
]
for col, (num, color, label) in zip([col1,col2,col3,col4], facts):
    col.markdown(f'''<div class="fact-card">
        <div class="fact-num" style="color:{color}">{num}</div>
...
st.markdown('''<div class="conclusion-box">
    <div style="color:#2ecc71; font-weight:700; font-size:1.1rem; margin-bottom:0.8rem">
...
st.markdown('''<div style="color:#718096; font-size:0.75rem; text-align:center; margin-top:1.5rem; line-height:1.8;">
    ...
    Agrofood CO2 Emissions · OECD Meat Consumption ·
    <a href="https://github.com/tejaswirupa/vizcon2026" style="color:#3498db">
    Full citations on GitHub</a>
</div>''', unsafe_allow_html=True)
